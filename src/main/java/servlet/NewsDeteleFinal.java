...
/**
 * Servlet implementation class NewsDeteleFinal
 */
@WebServlet("/NewsDeteleFinal")
public class NewsDeteleFinal extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NewsDeteleFinal() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(tokenBean.getsToken().equals(tokenBean.getToken())) {
			...
			try {
				...
				getServletContext().getRequestDispatcher("/WEB-INF/NewsDeleteComplete.jsp").forward(request, response);
