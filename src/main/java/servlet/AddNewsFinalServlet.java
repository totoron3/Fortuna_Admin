...
/**
 * Servlet implementation class AddNewsFinalServlet
 */
@WebServlet("/AddNewsFinalServlet")
public class AddNewsFinalServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddNewsFinalServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(tokenBean.getsToken());
		System.out.println(tokenBean.getToken());
		...
		if(tokenBean.getsToken().equals(tokenBean.getToken())) {
			...
			AddNewsBean anBean = (AddNewsBean)session.getAttribute("anBean");
			
			try {
				AddNewsDAO dao = new AddNewsDAO();
				dao.AddNews(anBean);
				
				getServletContext().getRequestDispatcher("/WEB-INF/AddNewsComplete.jsp").forward(request, response);
